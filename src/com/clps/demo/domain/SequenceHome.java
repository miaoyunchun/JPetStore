...
/**
 * Home object for domain model class Sequence.
 * @see com.clps.demo.domain.Sequence
 * @author Hibernate Tools
 */
public class SequenceHome {

	private static final Log log = LogFactory.getLog(SequenceHome.class);
	...
	public void persist(Sequence transientInstance) {
		log.debug("persisting Sequence instance");
		try {
	...
	public Sequence merge(Sequence detachedInstance) {
		...
		try {
			Sequence result = (Sequence) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Sequence findById(java.lang.String id) {
		...
		try {
			Sequence instance = (Sequence) sessionFactory.getCurrentSession().get("com.clps.demo.domain.Sequence", id);
			if (instance == null) {
	...
	public List findByExample(Sequence instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("com.clps.demo.domain.Sequence")
